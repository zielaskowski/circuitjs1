export class CircuitWS {
  ...
  async _ws_message(event) {
    ...
    if (!msg.hasOwnProperty("cmd")) {
      this._respond_error(
        "no_cmd_in_request",
        "No 'cmd' element found in JSON request.",
      );
      return;
    }
    ...
    if (!this.sim) {
      return this._respond_error(
        response,
        "no_sim_running",
        "No simulation running (iframe not loaded yet?).",
      );
    }
    ...
    if (msg.cmd == "status") {
      ...
      if (!msg.hasOwnProperty("timestep")) {
        return this._respond_error(
          response,
          "no_timestep_in_request",
          "No 'timestep' element found in JSON request.",
        );
      }
      ...
      if (!msg.hasOwnProperty("circuit")) {
        return this._respond_error(
          response,
          "no_circuit_in_request",
          "No 'circuit' element found in JSON request.",
        );
      }
  ...
  _on_iframe_load(event) {
    this._iframe.contentWindow.oncircuitjsloaded = (cjs) =>
      this._on_circuitjs_loaded(cjs);
  }

  _on_circuitjs_loaded(cjs) {
    this._respond_event("reload_complete");
    cjs.onsvgrendered = (cjs, svg_data) =>
      this._on_circuitjs_svg_rendered(cjs, svg_data);
