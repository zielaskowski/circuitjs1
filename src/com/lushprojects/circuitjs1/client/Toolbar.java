...
public class Toolbar extends FlowPanel {
	...
	// Translate touch gestures on a toolbar button into the synthetic mouse events
	// that
	// beginToolbarDrag/toolbarDragMove/toolbarDragEnd already expect, since
	// iOS/touch browsers
	// don't emit real mousemove events during a touch-drag (they just pan the page
	// instead).
	// touchstart re-dispatches a "mousedown" on the button itself, which the
	// existing
	// MouseDownHandler picks up; touchmove/touchend dispatch "mousemove"/"mouseup"
	// on the
	// document so UIManager's global NativePreviewHandler (which drives the drag
	// once it's
